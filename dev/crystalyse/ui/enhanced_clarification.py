...
from ..workspace.workspace_tools import ClarificationRequest, Question, QueryAnalysis
from .user_preference_memory import UserPreferenceMemory, UserInteractionRecord
...
class IntegratedClarificationSystem:
    ...
    async def analyze_and_clarify(self, query: str, request: ClarificationRequest, current_mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Query analysis: expertise={analysis.expertise_level}, specificity={analysis.specificity_score:.2f}, domain_confidence={analysis.domain_confidence:.2f}")
        
        # 2. Get personalized strategy based on user history
        personalized_strategy = self.preference_memory.get_personalized_strategy(self.user_id, analysis)
        ...
        strategy_name = personalized_strategy.get("clarification_method", 
                                                 self._select_clarification_strategy(analysis))
        ...
        logger.info(f"Selected clarification strategy: {strategy_name} for expertise: {analysis.expertise_level}")
        ...
        self._record_interaction_for_learning(query, analysis, strategy_name, 
                                            clarified_answers, interaction_time)
        
        return clarified_answers

    def _select_clarification_strategy(self, analysis: Optional[QueryAnalysisResponse]) -> str:
        """Choose clarification approach based on user analysis."""
        if analysis is None:
            return "focused_questions"  # Safe default
            
        if (analysis.expertise_level == "expert" and 
            analysis.specificity_score > 0.8 and
    ...
    async def _confirm_assumptions(self, request: ClarificationRequest, analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
        ...
        suggested_mode = analysis.suggested_mode.value if analysis and analysis.suggested_mode else "adaptive"
    ...
    async def _guide_discovery(self, request: ClarificationRequest, analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
        ...
        answers = {"_mode": mode, "_method": "guided_discovery", "_user_type": analysis.expertise_level}
        ...
        for question in request.questions:
            ...
            if question.options:
                ...
                answer = Prompt.ask("Your choice", choices=simplified_options, default=simplified_options[0])
    ...
    async def _focused_questions(self, request: ClarificationRequest, analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
        ...
        answers = {"_mode": "adaptive", "_method": "focused_questions", "_user_type": analysis.expertise_level}
        
        for question in request.questions:
            if question.options:
                likely_answer = likely_answers.get(question.id)
                default_answer = likely_answer if likely_answer in question.options else question.options[0]
                ...
                matched_answer = self._smart_response_matching(raw_answer, question.options, question.id)
                if matched_answer:
                ...
                answer = Prompt.ask(f"[bold]{question.text}[/bold]", default=likely_answers.get(question.id, ""))
                answers[question.id] = answer
        
        return answers

    def _smart_response_matching(self, response: str, options: List[str], question_id: str) -> Optional[str]:
    ...
    async def _call_llm_for_query_analysis(self, query: str) -> Optional[QueryAnalysisResponse]:
        """
...
- Low (0.0-0.3): General or unclear domain, misuse of technical terms"""
    ...
    async def _analyze_query_with_llm(self, query: str) -> Optional[QueryAnalysisResponse]:
        """
        ...
        try:
            ...
            logger.info(f"LLM analysis successful: {llm_analysis.expertise_level}, specificity={llm_analysis.specificity_score:.2f}")
    ...
    def _init_query_analysis_patterns(self):
        ...
        self.expertise_patterns = {
            "expert": [
                r"formation energy", r"phonon", r"dft", r"vasp", r"synthesis route",
                r"oxidation state", r"space group", r"lattice parameter", r"band gap",
                r"dos", r"bandstructure", r"thermodynamic", r"kinetic", r"crystallographic",
                # Thermoelectric terms
                r"thermoelectric", r"zt", r"seebeck", r"thermal conductivity", r"electrical conductivity",
                r"power factor", r"figure of merit", r"carrier concentration", r"mobility",
                # Crystal structure terms
                r"anisotrop", r"isotrop", r"crystal.*anisotrop", r"structural.*isotrop",
                r"polycrystalline", r"single.*crystal", r"grain.*boundar",
                # Processing terms
                r"sintering", r"hot.*press", r"densif", r"consolidat", r"bulk.*process",
                r"powder.*process", r"ceramic.*process",
                # Material classes
                r"zintl", r"skutterud", r"half.*heusler", r"chalcogenide", r"oxide.*thermoelectric",
                r"clathrate", r"perovskite", r"layered.*material",
                # Properties and characterization
                r"hall.*effect", r"resistivity", r"thermal.*diffusivity", r"specific.*heat",
                r"phonon.*scattering", r"electron.*transport", r"dopant", r"substitution",
                # Advanced concepts
                r"nanostructur", r"quantum.*well", r"superlattice", r"interfacial.*resist",
                r"phonon.*engineer", r"band.*engineer", r"defect.*engineer",
                # Battery-specific expert terms
                r"spinel.*struct", r"olivine.*struct", r"layered.*oxide", r"theoretical.*capacity",
                r"operating.*voltage", r"vs.*mg", r"vs.*li", r"electrolyte.*compat", r"carbonate.*electrolyte",
                r"mah/g", r"wh/kg", r"c-rate", r"coulombic.*efficiency", r"capacity.*retention",
                r"voltage.*fade", r"impedance.*growth", r"side.*reaction", r"sei.*formation",
                # Advanced characterization
                r"xrd.*pattern", r"xps.*spectr", r"sem.*morphol", r"tem.*analys", r"nmr.*spectr",
                r"electrochemical.*impedance", r"cyclic.*voltamm", r"galvanostatic", r"potentiostatic"
            ],
            "intermediate": [
                r"structure.*prediction", r"stability", r"composition", r"crystal.*system",
                r"properties", r"synthesis", r"fabrication", r"characterization",
                # Battery intermediate terms
                r"cathode.*material", r"anode.*material", r"electrolyte", r"separator",
                r"li.*ion", r"na.*ion", r"mg.*ion", r"capacity", r"voltage", r"energy.*density",
                r"cycle.*life", r"charging", r"discharge", r"battery.*type", r"cell.*design",
                # Materials intermediate terms
                r"crystal.*structure", r"lattice.*parameter", r"phase.*transition", r"dopant",
                r"substitution", r"solid.*solution", r"composite", r"coating", r"surface.*modif"
            ],
            "novice": [
                r"^what.*material", r"^find.*material", r"^suggest", r"^recommend", 
                r"^help.*choose", r"beginner", r"^i.*need.*help",
                r"^can.*you.*help", r"^simple.*material", r"^basic.*material",
                r"^easy.*to.*make", r"^cheap.*material", r"^common.*material"
            ]
        }
        
        self.urgency_patterns = [
            r"quick", r"fast", r"urgent", r"asap", r"deadline", r"immediately",
            r"need.*soon", r"time.*sensitive", r"rushing", r"hurry"
        ]
        
        self.complexity_indicators = {
            "safety_critical": [r"create.*toxic", r"synthesize.*explosive", r"nuclear.*material", r"radioactive", r"highly.*toxic", r"dangerous.*synthesis"],
    ...
    def _detect_expertise_level(self, query_lower: str) -> str:
        ...
        expert_score = sum(1 for pattern in self.expertise_patterns["expert"] 
                          if re.search(pattern, query_lower))
        intermediate_score = sum(1 for pattern in self.expertise_patterns["intermediate"]
                               if re.search(pattern, query_lower))
    ...
    def _calculate_specificity(self, query: str) -> float:
        ...
        general_count = sum(1 for pattern in general_patterns
    ...
    def _assess_domain_confidence(self, query_lower: str) -> float:
        ...
        materials_terms = [
            "material", "crystal", "structure", "element", "compound", "synthesis",
            "properties", "electronic", "thermal", "mechanical", "stability",
            "formation", "energy", "bandgap", "conductivity", "thermoelectric",
            # Thermoelectric specific terms
            "zt", "seebeck", "power factor", "figure of merit", "thermal conductivity",
            "electrical conductivity", "carrier", "mobility", "doping",
            # Crystal/structure terms
            "isotrop", "anisotrop", "polycrystalline", "sintering", "processing",
            "fabrication", "bulk", "grain", "phase", "lattice",
            # Performance terms
            "performance", "efficient", "optimize", "enhance", "improve",
            # Materials classes
            "oxide", "chalcogenide", "alloy", "ceramic", "semiconductor",
            # Chemical elements and formulas (broad patterns)
            "pb", "te", "se", "sn", "lead", "tellurium", "selenium", "tin"
        ]
    ...
    def _classify_interaction_style(self, query_lower: str, complexity_factors: Dict[str, bool]) -> str:
    ...
    async def _determine_mode_from_responses(self, answers: Dict[str, Any], analysis: Optional[QueryAnalysisResponse]) -> str:
        ...
        rigorous_keywords = ["validate", "verify", "thorough", "accurate", "precise", 
                           "publication", "research", "rigorous", "comprehensive"]
        rigorous_signals += sum(1 for kw in rigorous_keywords if kw in answers_text)
        
        # Creative mode indicators
        creative_keywords = ["explore", "discover", "quick", "fast", "novel", 
                           "innovative", "alternatives", "possibilities"]
    ...
    def _suggest_initial_mode(self, analysis: Optional[QueryAnalysisResponse]) -> str:
        """
    ...
    def _record_interaction_for_learning(self, 
                                         query: str,
                                         analysis: Optional[QueryAnalysisResponse],
        ...
        try:
            ...
            logger.debug(f"Recorded interaction for learning: {clarification_method} -> {result.get('_mode')}")
            
        except Exception as e:
            logger.warning(f"Failed to record interaction for learning: {e}")
    
    def record_user_feedback(self, feedback: str, satisfaction_score: Optional[float] = None):
        ...
        try:
            ...
            if satisfaction_score is None:
                ...
                if any(word in feedback_lower for word in ["great", "perfect", "excellent", "good"]):
                    satisfaction_score = 0.9
                elif any(word in feedback_lower for word in ["wrong", "bad", "terrible", "useless"]):
    ...
    async def _generate_smart_assumptions(self, questions: List[Question], analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
        ...
        try:
            response = await o3_mini_client.chat.completions.create(
                model="o4-mini",
                messages=[
                    {"role": "system", "content": """You are a materials science expert helping to generate smart assumptions for clarification questions.
    ...
    def _generate_fallback_assumptions(self, questions: List[Question], analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
        ...
        for q in questions:
            ...
            # Constraint assumptions
                ...
                if analysis and any("lead" in term.lower() or "tellurium" in term.lower() for term in analysis.technical_terms):
            ...
            # Fabrication assumptions
                ...
                if analysis and any("sintering" in term.lower() or "polycrystalline" in term.lower() for term in analysis.technical_terms):
            ...
            # Material class assumptions
                ...
                if analysis and any("thermoelectric" in term.lower() for term in analysis.technical_terms):
    ...
    def _generate_likely_answers(self, questions: List[Question], analysis: Optional[QueryAnalysisResponse]) -> Dict[str, Any]:
