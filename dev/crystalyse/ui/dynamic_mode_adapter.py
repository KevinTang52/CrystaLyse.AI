...
class DynamicModeAdapter:
    ...
    def __init__(self):
        ...
        self.adaptation_signals = {
            "wants_speed": {
                "patterns": ["faster", "quicker", "taking too long", "speed up", 
                            "just give me", "hurry", "quick results", "skip details"],
                "target_mode": "creative",
                "confidence": 0.8
            },
            "wants_depth": {
                "patterns": ["more detail", "deeper", "validate", "verify", "research",
                            "thorough", "comprehensive", "explain more", "check this"],
                "target_mode": "rigorous",
                "confidence": 0.85
            },
            "wants_simplicity": {
                "patterns": ["simpler", "confusing", "too technical", "basic", "easier",
                            "don't understand", "lost", "explain simply", "layman"],
                "target_mode": "creative",
                "confidence": 0.7
            },
            "satisfied": {
                "patterns": ["perfect", "exactly", "that's right", "good", "thanks",
                            "great", "awesome", "yes that's it"],
                "target_mode": None,  # No change
                "confidence": 0.9
            },
            "frustrated": {
                "patterns": ["wrong", "not what I", "incorrect", "no that's not",
    ...
    async def monitor_and_adapt(self, 
                               ...
                               user_feedback: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def _check_automatic_conditions(self, 
                                   current_mode: str,
                                   context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Check for conditions that warrant automatic mode switching."""
        
        # Low confidence in creative mode → Switch to rigorous
        if (current_mode == "creative" and 
            context.get("confidence_score", 1.0) < 0.5 and
            not context.get("time_critical", False)):
        ...
        if (current_mode != "rigorous" and 
            context.get("novel_chemistry_detected", False)):
        ...
        if (context.get("safety_critical_detected", False) and 
    ...
    def _analyze_user_feedback(self, 
                              feedback: str,
                              current_mode: str) -> Optional[Dict[str, Any]]:
    ...
    def _check_performance_metrics(self,
        ...
        if (execution_time > expected_time * 2 and 
            current_mode == "rigorous" and
            not context.get("complexity_acknowledged", False)):
    ...
    def get_adaptation_statistics(self) -> Dict[str, Any]:
        ...
        stats["average_confidence"] = sum(e.confidence for e in self.adaptation_history) / len(self.adaptation_history)
...
class AdaptiveExecutionMonitor:
    ...
    async def check_adaptation_needed(self, checkpoint: str, metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    async def apply_user_feedback(self, feedback: str) -> Optional[Dict[str, Any]]:
        """
