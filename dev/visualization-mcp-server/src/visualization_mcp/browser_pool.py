...
class BrowserSessionManager:
    ...
    def _get_or_create_session(self):
        ...
        with self.session_lock:
            ...
            if (self.browser_session is not None and 
                current_time - self.last_used < self.session_timeout):
        ...
        with self.batch_session():
            ...
            for i, (figure, output_path) in enumerate(figures_and_paths):
                try:
                    ...
                    logger.info(f"Figure {i+1}/{len(figures_and_paths)} saved: {Path(output_path).name}")
