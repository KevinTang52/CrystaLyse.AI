...
class PersistentSession:
        ...
        try:
            for server_name, config in server_configs.items():
                ...
                if connection:
                    ...
                    logger.warning(f"⚠️ Session {self.session_id}: Failed to connect to {server_name}")
    ...
    def record_tool_call(self, tool_name: str, success: bool, operation: str) -> None:
        ...
        self.context.tool_calls_count[tool_name] = self.context.tool_calls_count.get(tool_name, 0) + 1
        ...
        return {
            ...
            'tool_usage': {
                ...
                'success_rate': (
                    ...
                    (len(self.context.successful_operations) + len(self.context.failed_operations))
                ) if (len(self.context.successful_operations) + len(self.context.failed_operations)) > 0 else 0
            },
...
class PersistentSessionManager:
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sessions': {
                user_id: session.get_context_summary() 
                for user_id, session in self.sessions.items()
            }
        }
